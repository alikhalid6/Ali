// This file contains assets and translations for the application.

// 1. ASSETS
// ==========================================

// URL for the logo image
export const BAHI_LOGO_URL = 'https://i.postimg.cc/sgkktgNq/bahilogo.png';

// SVG icons
export const HEART_SVG = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>`;
export const HEART_FILLED_SVG = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clip-rule="evenodd" /></svg>`;
export const SHARE_SVG = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12s-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6.002l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.368a3 3 0 105.367 2.684 3 3 0 00-5.367 2.684z" /></svg>`;
export const OMAN_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36"><path fill="#FFF" d="M0 0h36v36H0z"/><path fill="#D21034" d="M12 0h24v36H12z"/><path fill="#007A3D" d="M12 18h24v18H12z"/><path fill="#D21034" d="M0 0h12v36H0z"/><path fill="#FFF" d="M5.14 5.14a4.86 4.86 0 0 0 0 6.88 4.86 4.86 0 0 0 6.88 0l-1.02-1.02a3.44 3.44 0 0 1-4.85 0 3.44 3.44 0 0 1 0-4.85 3.44 3.44 0 0 1 4.85 0l1.02-1.02a4.86 4.86 0 0 0-6.88 0z"/><path fill="#FFF" d="M6 3.1h5.1v2H7.72L10.3 7.7a.7.7 0 0 1 0 1 .7.7 0 0 1-1 0L7.04 6.43V9.1H5.1V3.1H6zm.5 10.95c.3-.1.6-.2.9-.2s.6.1.9.2l.4.4-1.3 1.3-1.3-1.3.4-.4z"/><path fill="#D21034" d="M7.78 11.45a3.44 3.44 0 0 0 4.85 0l-1.12-1.12a2 2 0 0 1-2.6 0l-1.13 1.12z"/><path fill="#FFF" d="M9.82 8.7a.7.7 0 0 1-1 0L6.5 6.4v3.1h-2v-7h7v2H8.38l2.32 2.3a.7.7 0 0 1 0 1zM6 10.08h5.1v1.88H6zm3.9-3.78a2.43 2.43 0 1 0-3.44 0 2.43 2.43 0 0 0 3.44 0z"/></svg>`;
export const USA_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 36 36"><path fill="#B22234" d="M32 5H0v2H0v2H0v2H0v2h32V5zm0 6H0v2H0v2h32v-4zM0 21h32v-2H0v-2H0v4zm0 6h32v-2H0v-2H0v4z"/><path fill="#3C3B6E" d="M0 5h18v14H0z"/><path fill="#FFF" d="m4 6 1 3h3l-2 2 1 3-3-2-3 2 1-3-2-2h3z m10 0 1 3h3l-2 2 1 3-3-2-3 2 1-3-2-2h3z m-5 3.5 1 3h3l-2 2 1 3-3-2-3 2 1-3-2-2h3z m10 0 1 3h3l-2 2 1 3-3-2-3 2 1-3-2-2h3z m-5 3.5 1 3h3l-2 2 1 3-3-2-3 2 1-3-2-2h3z"/></svg>`;
export const SAUDI_ARABIA_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600"><rect width="900" height="600" fill="#006c35"/><text x="450" y="350" font-family="Arial, sans-serif" font-size="120" fill="#fff" text-anchor="middle">لا إله إلا الله محمد رسول الله</text><path d="M150 450h600l-30 20h-540z m0 30h600v20h-600z" fill="#fff"/></svg>`;
export const UAE_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 400"><path fill="#00732f" d="M0 0h800v133.3H0z"/><path d="M0 133.3h800v133.4H0z"/><path fill="#000" d="M0 266.7h800V400H0z"/><path fill="red" d="M0 0h200v400H0z"/></svg>`;
export const QATAR_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 110"><path fill="#fff" d="M0 0h280v110H0z"/><path fill="#8d1b3d" d="M80 0h200v110H80L60 5 80 10l-20 5L80 20l-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5 20 5-20 5L80 90l-20-5L80 100l-20-5 20 5z"/></svg>`;
export const KUWAIT_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 60"><path d="M0 0h120v20H0z"/><path fill="#007a3d" d="M0 20h120v20H0z"/><path fill="red" d="M0 40h120v20H0z"/><path d="M0 0l30 20v20L0 60z"/></svg>`;
export const BAHRAIN_FLAG_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 60"><path fill="#ce1126" d="M0 0h100v60H0z"/><path fill="#fff" d="M0 0h40l10 6-10 6 10 6-10 6 10 6-10 6 10 6-10 6 10 6-10 6h-40z"/></svg>`;
export const INSTAGRAM_SVG = `<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  <rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect>
  <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
  <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>
</svg>`;
export const WHATSAPP_SVG = `<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
  <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.894 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.886-.001 2.267.655 4.398 1.908 6.161l-1.317 4.814 4.893-1.282zM9.062 8.611c-.021-.123-.16-.2-.355-.2-.178 0-.374-.01-.546-.01-.221 0-.441.031-.64.153-.258.157-.492.355-.67.604-.213.292-.354.626-.354 1.014 0 .428.149.82.368 1.15.228.347.505.67.81 1.005.348.379.728.742 1.13 1.09.522.455 1.01.812 1.516 1.066.592.302 1.18.428 1.76.428.324 0 .61-.051.84-.123.308-.092.64-.289.87-.589.255-.325.395-.733.395-1.178 0-.254-.04-.498-.12-.732-.08-.234-.23-.429-.44-.574-.21-.145-.44-.22-.7-.22-.26 0-.5.061-.71.183s-.38.28-.51.465c-.13.184-.24.325-.33.428-.109.122-.21.183-.31.183-.1 0-.18-.03-.26-.1-.1-.06-.25-.151-.45-.272-.34-.213-.64-.498-.89-.832-.23-.326-.39-.641-.46-.948-.07-.307-.03-.565.04-.773.07-.208.19-.375.36-.498.17-.123.33-.183.49-.183.16 0 .31.041.45.123s.25.184.34.305c.09.123.16.2.22.2.06 0 .12-.01.18-.04s.17-.071.25-.122c.08-.051.15-.112.21-.183.06-.071.1-.152.12-.243.02-.092.02-.173.01-.244-.01-.071-.02-.132-.04-.183-.02-.051-.05-.102-.09-.153z"></path>
</svg>`;
export const VISA_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="#1A1F71" d="M15.34.66h-5.9l-2.67 16.2h6.03c.5 0 .9-.08 1.2-.23.3-.15.55-.38.74-.68.2-.3.34-.67.42-1.08.08-.42.12-.87.12-1.35 0-.6-.1-1.1-.3-1.5-.2-.4-.48-.74-.82-1-.35-.28-.76-.48-1.2-.6.9-.27 1.6-.8 2.1-1.55.5-.75.75-1.65.75-2.7 0-1.1-.3-2.02-.88-2.77-.6-.74-1.4-1.12-2.42-1.12zM12.9 7.3c.2.14.35.35.45.63.1.28.15.62.15 1.02 0 .42-.05.78-.15 1.08-.1.3-.25.54-.45.72-.2.18-.45.28-.75.28h-1.6l1.3-4.5c.2-.7.4-.8.9-.8.34 0 .6.1.8.25zm-.3-4.1c.15.1.26.28.34.52.08.24.12.5.12.8 0 .34-.04.64-.12.88-.08.24-.2.43-.34.55-.15.12-.33.18-.54.18h-1.2L12.1 3c.18-.6.3-.72.8-.72.3 0 .54.05.6.14z"/><path fill="#1A1F71" d="M22.06 13.9c.2-.2.38-.43.52-.68.14-.26.26-.52.34-.8.1-.28.14-.58.14-.9 0-.5-.1-.94-.3-1.32-.2-.4-.5-.72-.9-.95-.38-.23-.84-.35-1.36-.35-.58 0-1.1.1-1.55.3-.45.2-.84.48-1.15.82-.3.34-.54.75-.7 1.2-.17.48-.25.98-.25 1.5s.08 1.02.24 1.5c.16.5.4.92.7 1.25.3.34.67.6 1.1.78.4.18.9.28 1.45.28.46 0 .88-.07 1.25-.22s.68-.35 1-.6zM19.3 12.4c-.1-.2-.15-.42-.15-.65 0-.22.05-.43.15-.62s.22-.36.38-.5c.15-.14.33-.2.52-.2.2 0 .37.07.5.2s.2.3.2.52c0 .24-.04.45-.12.63s-.2.35-.35.5c-.15.15-.32.22-.5.22-.2 0-.38-.07-.53-.22z"/><path fill="#1A1F71" d="M8.2 11.82c0 .85-.13 1.57-.4 2.15-.27.6-.66.9-1.15.9-.2 0-.4-.03-.58-.1-.2-.07-.35-.18-.48-.32l-.63-2.1-1.32 2.3c-.14.24-.32.42-.55.54-.22.12-.5.18-.8.18h-.18c-.4 0-.7-.1-.9-.3-.2-.2-.3-.5-.3-.92L2.5 1.16C2.42.56 2.5 0 3.2 0h2.4c.4 0 .73.12.95.35.23.23.34.55.34.95l-1.8 9.2.02.1c.1.2.25.3.45.3.2 0 .4-.1.6-.2l2.3-3.2c.1-.15.2-.26.3-.35.1-.08.2-.12.3-.12.4 0 .4.4.4 1.1z"/></svg>`;


// 2. TRANSLATIONS
// ==========================================

export const translations = {
  ar: {
    brandName: 'باهي',
    // Header & Menu
    menuHome: 'الرئيسية',
    menuAbout: 'عنا',
    menuContact: 'تواصل معنا',
    menuWishlist: 'المفضلة',
    menuFeedback: 'رأيك يهمنا',
    menuLanguage: 'اللغة',
    menuMyOrders: 'طلباتي',

    // Hero Section
    heroTitle: 'تخفيضات نهاية رمضان',
    heroSubtitle: 'اكتشف أحدث المصار  بأسعار لا تقاوم. خصومات تصل إلى 50% على تشكيلة مختارة.',
    heroButton: 'تسوق الآن',

    // Product List
    newArrivals: 'وصل حديثاً',

    // Product Card
    sale: 'تخفيض',
    addToCart: 'أضف إلى السلة',
    currency: 'ر.ع.',

    // Product Page
    quantity: 'الكمية',
    inStock: 'متوفر',
    outOfStock: 'نفذت الكمية',
    addToWishlist: 'أضف إلى المفضلة',
    addedToWishlist: 'في المفضلة',
    sku: 'رمز المنتج',
    category: 'التصنيف',
    tags: 'الوسوم',
    share: 'مشاركة',
    description: 'الوصف',
    relatedProducts: 'منتجات ذات صلة',

    // Cart
    cartTitle: 'سلة التسوق',
    cartTooltip: 'سلة المشتريات',
    cartDiscountPlaceholder: 'أدخل كود الخصم',
    cartApply: 'تطبيق',
    cartInvalidCode: 'كود الخصم غير صالح.',
    cartCodeSuccess: 'تم تطبيق كود الخصم بنجاح!',
    cartSubtotal: 'المجموع الفرعي',
    cartDiscount: 'الخصم',
    cartTotal: 'المجموع الإجمالي',
    cartCheckout: 'إتمام الشراء',
    cartEmptyTitle: 'سلتك فارغة',
    cartEmptySubtitle: 'ابدأ بإضافة بعض المنتجات الرائعة!',
    
    // Promo Message
    promoMessage: 'اختار مصر ثاني والتوصيل مجاني',
    promoMessageFreeDelivery: 'مبرووك التوصيل مجاني يا وحش',

    // Wishlist Page
    wishlistTitle: 'قائمة المفضلة',
    wishlistEmptyTitle: 'قائمة المفضلة فارغة',
    wishlistEmptySubtitle: 'أضف المنتجات التي تعجبك لتبقى على إطلاع دائم بها.',

    // Testimonials
    testimonialsTitle: 'آراء زبائن باهي',
    testimonialsSubtitle: 'آراء',
    customer: 'زبون باهي',

    // Footer
    footerSubtitle: 'وجهتك الأولى للأناقة.',
    contact: 'التواصل',
    footerRights: 'جميع الحقوق محفوظة.',
    
    // About Page
    aboutTitle: 'عنا',
    aboutText: 'هذه هي صفحة عنا. سيتم تحديث المحتوى قريباً.',

    // Contact Page
    contactTitle: 'تواصل معنا',
    contactSubtitle: 'كيف يمكننا المساعدة؟',
    contactName: 'اسمك الكريم',
    contactPhone: 'رقمك للتواصل',
    contactEmail: 'ايميلك للتواصل',
    contactMessage: 'رسالتك',
    contactSend: 'إرسال الرسالة',

    // Feedback Page
    feedbackTitle: 'رأيك يهمنا',
    feedbackSubtitle: 'شاركنا رأيك لمساعدتنا على تحسين خدماتنا.',
    feedbackName: 'اسمك الكريم',
    feedbackPhone: 'رقمك للتواصل (اختياري)',
    feedbackMessage: 'رأيك',
    feedbackSend: 'إرسال الرأي',

    // Checkout Page
    checkoutTitle: 'الدفع',
    checkoutDiscountHeader: 'هل لديك قسيمة؟ أنقر هنا لإدخال رمز القسيمة',
    checkoutFirstName: 'الاسم الأول',
    checkoutLastName: 'الاسم الأخير',
    checkoutCountry: 'الدولة / المنطقة',
    checkoutStreetAddress: 'عنوان الشارع / الحي',
    checkoutStreetAddressPlaceholder: 'رقم المنزل واسم الشارع / الحي',
    checkoutCity: 'المدينة',
    checkoutRegion: 'المنطقة',
    checkoutPhone: 'الهاتف',
    checkoutEmail: 'البريد الإلكتروني',
    checkoutCreateAccount: 'هل تود إنشاء حساب جديد؟',
    checkoutShipToDifferent: 'هل تود الشحن لعنوان مختلف؟',
    checkoutIsGift: 'هل هذه هدية؟ (اختياري)',
    checkoutOrderNotes: 'ملاحظات حول الطلب (اختياري)',
    checkoutOrderNotesPlaceholder: 'ملاحظة خاصة بتسليم الطلب.',
    checkoutTerms: 'لقد قرأت الشروط والأحكام وأوافق عليها لهذا الموقع',
    checkoutPlaceOrder: 'تأكيد الطلب',
    checkoutOrderSummary: 'ملخص الطلب',
    checkoutProduct: 'المنتج',
    checkoutShipping: 'الشحن',
    checkoutTotal: 'الإجمالي',
    checkoutPaymentMethod: 'طريقة الدفع',
    paymentMethodCard: 'بطاقة بنكية',
    paymentMethodApplePay: 'Apple Pay',
    checkoutShippingHome: 'التوصيل للمنزل',
    checkoutShippingPickup: 'الإستلام من المكتب',

    // Payment Page
    paymentTitle: 'إتمام عملية الدفع',
    paymentTotal: 'المبلغ الإجمالي للدفع',
    paymentPayWithCard: 'البطاقة البنكية',
    paymentApplePay: 'Apple Pay',
    paymentCardNumber: 'رقم البطاقة',
    paymentCardHolder: 'اسم صاحب البطاقة',
    paymentExpiryDate: 'تاريخ الإنتهاء',
    paymentCVC: 'CVC',
    paymentPayNow: 'ادفع الآن',
    paymentProcessing: 'جاري معالجة الدفع...',
    paymentWait: 'يرجى الإنتظار، لا تغلق الصفحة.',
    paymentSuccessful: 'تم الدفع بنجاح!',
    paymentRedirecting: 'شكراً لطلبك. سيتم تحويلك للصفحة الرئيسية الآن.',
    paymentApplePayInstruction: 'انقر على الزر أدناه للدفع باستخدام Apple Pay.',
    invalidCardNumber: 'رقم بطاقة الفيزا غير صالح.',

    // Order History & Confirmation
    orderHistoryTitle: 'سجل الطلبات',
    orderConfirmationTitle: 'تأكيد الطلب',
    thankYouMessage: 'شكراً لطلبك!',
    orderConfirmationSubtitle: 'لقد استلمنا طلبك وسنقوم بمعالجته قريباً.',
    orderNumber: 'رقم الطلب',
    orderIdLabel: 'رقم الطلب',
    orderDateLabel: 'تاريخ الطلب',
    orderStatusLabel: 'حالة الطلب',
    date: 'التاريخ',
    status: 'الحالة',
    total: 'الإجمالي',
    viewOrder: 'عرض التفاصيل',
    noOrders: 'لا يوجد طلبات بعد',
    noOrdersSubtitle: 'عندما تقوم بطلب، ستجده هنا.',
    orderNotFound: 'لم يتم العثور على الطلب.',
    processing: 'قيد المعالجة',
    shipped: 'تم الشحن',
    delivered: 'تم التوصيل',
    goToHomepage: 'الانتقال الى الصفحة الرئيسية',
  },
  en: {
    brandName: 'Bahi',
    // Header & Menu
    menuHome: 'Home',
    menuAbout: 'About Us',
    menuContact: 'Contact Us',
    menuWishlist: 'Wishlist',
    menuFeedback: 'Your Feedback',
    menuLanguage: 'Language',
    menuMyOrders: 'My Orders',

    // Hero Section
    heroTitle: 'End of Season Sale',
    heroSubtitle: 'Discover the latest fashion trends at irresistible prices. Discounts up to 50% on selected items.',
    heroButton: 'Shop Now',

    // Product List
    newArrivals: 'New Arrivals',

    // Product Card
    sale: 'SALE',
    addToCart: 'Add to Cart',
    currency: 'OMR',

    // Product Page
    quantity: 'Quantity',
    inStock: 'In Stock',
    outOfStock: 'Out of Stock',
    addToWishlist: 'Add to Wishlist',
    addedToWishlist: 'In Wishlist',
    sku: 'SKU',
    category: 'Category',
    tags: 'Tags',
    share: 'Share',
    description: 'Description',
    relatedProducts: 'Related Products',

    // Cart
    cartTitle: 'Shopping Cart',
    cartTooltip: 'Shopping Cart',
    cartDiscountPlaceholder: 'Enter discount code',
    cartApply: 'Apply',
    cartInvalidCode: 'Invalid discount code.',
    cartCodeSuccess: 'Discount code applied successfully!',
    cartSubtotal: 'Subtotal',
    cartDiscount: 'Discount',
    cartTotal: 'Total',
    cartCheckout: 'Proceed to Checkout',
    cartEmptyTitle: 'Your cart is empty',
    cartEmptySubtitle: 'Start adding some amazing products!',

    // Promo Message
    promoMessage: 'Add one more for FREE delivery!',
    promoMessageFreeDelivery: 'Congrats! Free delivery unlocked!',
    
    // Wishlist Page
    wishlistTitle: 'My Wishlist',
    wishlistEmptyTitle: 'Your Wishlist is Empty',
    wishlistEmptySubtitle: 'Add products you love to keep track of them here.',

    // Testimonials
    testimonialsTitle: 'Customer Reviews',
    testimonialsSubtitle: 'Reviews',
    customer: 'Bahi Customer',

    // Footer
    footerSubtitle: 'Your first destination for elegance.',
    contact: 'Contact',
    footerRights: 'All rights reserved.',

    // About Page
    aboutTitle: 'About Us',
    aboutText: 'This is the About Us page. Content will be updated soon.',

    // Contact Page
    contactTitle: 'Contact Us',
    contactSubtitle: 'How can we help?',
    contactName: 'Your Name',
    contactPhone: 'Your Phone Number',
    contactEmail: 'Your Email',
    contactMessage: 'Your Message',
    contactSend: 'Send Message',

    // Feedback Page
    feedbackTitle: 'We Value Your Feedback',
    feedbackSubtitle: 'Share your thoughts to help us improve our services.',
    feedbackName: 'Your Name',
    feedbackPhone: 'Your Phone Number (Optional)',
    feedbackMessage: 'Your Feedback',
    feedbackSend: 'Send Feedback',

    // Checkout Page
    checkoutTitle: 'Checkout',
    checkoutDiscountHeader: 'Have a coupon? Click here to enter your code',
    checkoutFirstName: 'First Name',
    checkoutLastName: 'Last Name',
    checkoutCountry: 'Country / Region',
    checkoutStreetAddress: 'Street address',
    checkoutStreetAddressPlaceholder: 'House number and street name',
    checkoutCity: 'Town / City',
    checkoutRegion: 'Region',
    checkoutPhone: 'Phone',
    checkoutEmail: 'Email address',
    checkoutCreateAccount: 'Create an account?',
    checkoutShipToDifferent: 'Ship to a different address?',
    checkoutIsGift: 'Is this a gift? (optional)',
    checkoutOrderNotes: 'Order notes (optional)',
    checkoutOrderNotesPlaceholder: 'Notes about your order, e.g. special notes for delivery.',
    checkoutTerms: 'I have read and agree to the website terms and conditions',
    checkoutPlaceOrder: 'Place Order',
    checkoutOrderSummary: 'Your Order',
    checkoutProduct: 'Product',
    checkoutShipping: 'Shipping',
    checkoutTotal: 'Total',
    checkoutPaymentMethod: 'Payment Method',
    paymentMethodCard: 'Bank Card',
    paymentMethodApplePay: 'Apple Pay',
    checkoutShippingHome: 'Home Delivery',
    checkoutShippingPickup: 'Pickup from Office',

    // Payment Page
    paymentTitle: 'Complete Payment',
    paymentTotal: 'Total amount to pay',
    paymentPayWithCard: 'Credit / Debit Card',
    paymentApplePay: 'Apple Pay',
    paymentCardNumber: 'Card Number',
    paymentCardHolder: 'Cardholder Name',
    paymentExpiryDate: 'Expiry Date',
    paymentCVC: 'CVC',
    paymentPayNow: 'Pay Now',
    paymentProcessing: 'Processing Payment...',
    paymentWait: 'Please wait, do not close the page.',
    paymentSuccessful: 'Payment Successful!',
    paymentRedirecting: 'Thank you for your order. You will now be redirected.',
    paymentApplePayInstruction: 'Click the button below to pay with Apple Pay.',
    invalidCardNumber: 'Invalid Visa card number.',

    // Order History & Confirmation
    orderHistoryTitle: 'Order History',
    orderConfirmationTitle: 'Order Confirmation',
    thankYouMessage: 'Thank you for your order!',
    orderConfirmationSubtitle: 'We have received your order and will process it shortly.',
    orderNumber: 'Order No.',
    orderIdLabel: 'Order ID',
    orderDateLabel: 'Order Date',
    orderStatusLabel: 'Order Status',
    date: 'Date',
    status: 'Status',
    total: 'Total',
    viewOrder: 'View Details',
    noOrders: 'You have no orders yet',
    noOrdersSubtitle: 'When you place an order, it will appear here.',
    orderNotFound: 'Order not found.',
    processing: 'Processing',
    shipped: 'Shipped',
    delivered: 'Delivered',
    goToHomepage: 'Go to Homepage',
  },
};